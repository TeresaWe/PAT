...
file_list_raw<-AL10KON175_sound_list
dev_sounds_AL10KON175 <- list()

for (j in seq_along(file_list_raw)) {
  #rm(dev_raw, vec, nam)
  dev_raw<-file_list_raw[[j]]
  PAT_table<-PAT_AL10KON175
  ...
  print(f+geom_line()+
          coord_cartesian(ylim=c(-1200,1200),xlim=c(0,170))) #two octaves
  #vec<-list(vec)
  dev_sounds_AL10KON175<-c(dev_sounds_AL10KON175,vec)
  j=j+1
}
