...
PAT_AR13ROB161 <- read.csv("~/files/PAT/IS17THE058_2018_Jan_24_1419/Absolute Pitch Experiment.csv")
...
for (i in seq_along(data_list)) {
  ...
  #following lines to reduce number of sample points (does ot work correctly)
  #seconds<-length(testwave)/44100 #devide by sample rate
  #steps<-length(freqWav2)/seconds 
  #index<-round(seq(1,length(freqWav), by=steps)) #round!
  #freq_reduced<-freqWav2[index]
  #noteWav2 <- noteFromFF(freq_reduced)
